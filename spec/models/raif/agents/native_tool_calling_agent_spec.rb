...
RSpec.describe Raif::Agents::NativeToolCallingAgent, type: :model do
  ...
  describe "#run!" do
    ...
    it "runs the agent" do
      stub_sentinel_agent(agent) do |_messages, model_completion|
    ...
    context "with multiple iterations" do
      ...
      it "processes multiple iterations until finding an answer" do
        ...
        expect(agent.sentinel_model_tool_invocations.length).to eq(1)
        mti = agent.sentinel_model_tool_invocations.first
  ...
  describe "validations" do
    it "validates that the LLM supports native tool use" do
      agent = described_class.new(
        creator: creator,
        task: "test",
        llm_model_key: "sentinel_test_llm"
      )
