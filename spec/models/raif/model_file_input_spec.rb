...
RSpec.describe Raif::ModelFileInput, type: :model do
  ...
  it "doesn't allow both input and URL" do
    expect do
      described_class.new(input: file_path, url: "https://example.com/file.pdf")
  ...
  it "raises an error when the file is missing" do
    expect do
      described_class.new(input: "missing.pdf")
    end.to raise_error(Raif::Errors::InvalidModelFileInputError, "File does not exist: missing.pdf")
  end
  ...
  it "handles URL input" do
    input = described_class.new(url: "https://example.com/file.pdf")
  ...
  it "handles a file path input" do
    ...
    expect(input.filename).to eq("test.pdf")
    expect(input.content_type).to eq("application/pdf")
  end
  ...
  it "handles an ActiveStorage::Attached object" do
    ...
    user.documents.attach(io: File.open(file_path), filename: "test.pdf", content_type: "application/pdf")
